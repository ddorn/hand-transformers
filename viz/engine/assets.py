...
@lru_cache()
def font(size: int = DEFAULT_TEXT_SIZE, name: str = None):
...
def text(txt, size, color, name=None):
    return font(size, name).render(txt, True, color)
...
@lru_cache()
def wrapped_text(txt: str, size, color, max_width, name=None, center=True):
    f = font(size, name)

    paragraphs = txt.splitlines()
    surfaces = []
    for paragraph in paragraphs:
        leading_space = len(paragraph) - len(paragraph.lstrip())
        leading_space = paragraph[:leading_space]
        offset = f.size(leading_space)[0]
        words = paragraph.split()
        while words:
            line = []
            while f.size(" ".join(line))[0] < max_width - offset:
                if not words:
                    break
            ...
            surfaces.append(
                text(leading_space + " ".join(line), size, color, name))
