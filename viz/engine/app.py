...
class App(StateMachine):
    ...
    def __init__(self, initial_state: Type[State], resizing: Screen, *args,
                 **kwargs):
        """
        :param initial_state: The first state to be run.
        :param resizing: The screen type to use.
        :param args: Arguments to pass to the initial state.
        :param kwargs: Keyword arguments to pass to the initial state.
        """
        pygame.init()
        App.MAIN_APP = self
    ...
    def run(self):
        ...
        duration = time() - start
        print(
            f"Game played for {duration:.2f} seconds, at {frame / duration:.1f} FPS."
        )
        settings.save()
    ...
    def quit(self, raise_system_exit: bool = True):
